...
export class TarotLLMService {
  ...
  private static async generateCardInterpretation(
    ...
    const avatar = (AVATAR_PROFILES as any)[avatarId];
    ...
    userPrompt += `POSITION: ${positionMeaning}\n`;
    userPrompt += `SPREAD TYPE: ${request.spreadType === 'daily' ? 'Daily Draw' : 'Three-Card Reading'}`;
  ...
  private static async generateSynthesis(
    ...
    const avatar = (AVATAR_PROFILES as any)[avatarId] as typeof AVATAR_PROFILES.olivia;
    ...
    userPrompt += cards.map((card, i) => `${i + 1}. ${card}`).join('\n');
   ...
   */
  private static generateReflectionPrompt(avatarId: string, spreadType: string): string {
  ...
  static async generateReading(request: ReadingGenerationRequest): Promise<{
    ...
    for (let i = 0; i < request.cards.length; i++) {
      ...
      const interpretation = await this.generateCardInterpretation(request.avatarId, {
        cardName: card.name,
        position: i,
    ...
    const overallGuidance = await this.generateSynthesis(request.avatarId, request.spreadType, cardNames, request.userQuestion);

    // Generate reflection prompt
    const reflectionPrompt = this.generateReflectionPrompt(request.avatarId, request.spreadType);
